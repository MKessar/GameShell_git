...
_mission_check() {
  ...
  #check if flame or cheater branch is in the history of beer
  id=$(git rev-parse flame)
  output=$(git log | grep $id)
  if [ "$output" != "" ]
  then
  ...
  id=$(git rev-parse cheater)
  output=$(git log | grep $id)
  if [ "$output" != "" ]
  then
    echo "Cheater branch is an ancestor of $expected_name branch but should not. You probably did not start the branch from main..."
    return 1
  fi
